...
<<<<<<< HEAD
   * @param angle_threshold Angle threshold (rad). If angle > threshold, no smoothing is applied.
   */
  CornerArc(
...
<<<<<<< HEAD
    // Cannot smooth a 0° angle (u-turn/back and forth) or
    // angles greater than angle_threshold (nearly straight line)
    if (std::abs(angle) < 1E-6 || std::abs(angle) > angle_threshold) {return;}
=======
...
protected:
  /**
<<<<<<< HEAD
   * @brief find the unsigned angle between a corner generated by 3 points
=======
